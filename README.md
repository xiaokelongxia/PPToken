...
- **账号管理** —— 多账号快照、导入导出、切换、退出和本地额度缓存。
- **线程管理** —— 读取 Codex `state_5.sqlite` 和 rollout 文件，支持归档、恢复和统计。
- **智能路由** —— Provider 管理、模型拉取、连通性测试、Codex 配置注入和诊断修复。
- **本地分析** —— 会话活跃度、Token、工具调用、读写变更和额度历史的本地统计接口。
- **内容后台** —— 顶栏消息、通知状态、反馈、神秘代码、远程设备 key 和插件本地配置。
...
> **合规说明：** PPToken 基于 Apache-2.0 许可的 AiMaMi 公开仓库衍生，保留上游版权声明；账号、线程、路由和后台能力由 PPToken 独立实现，不包含 AiMaMi 未公开的私有源码或反编译代码。开源发布流程见 [docs/OPEN_SOURCE.md](docs/OPEN_SOURCE.md)。
